...
def main():
    ...
    if not geoms: raise RuntimeError('no mesh geometry in Shrek FBX')
    ...
    geom_base={}; all_bind=[]; geom_sizes={}
    for g in geoms:
        gid=g.props[0]; verts=np.array(child(g,'Vertices').props[0],float).reshape(-1,3); geom_base[gid]=len(all_bind); geom_sizes[gid]=len(verts)
    ...
    body_gid=max(geom_sizes, key=geom_sizes.get)
    body_img=Image.open(a.body_texture).convert('RGBA')
    head_img=Image.open(a.head_texture).convert('RGBA')
    AW=max(body_img.width,head_img.width); AH=body_img.height+head_img.height
    atlas=Image.new('RGBA',(AW,AH),(0,0,0,0))
    atlas.paste(body_img,(0,0)); atlas.paste(head_img,(0,body_img.height))

    # Shrek is split across several geometries: the main body mesh plus an extra
    # accessory/trim mesh both belong on the body texture, while the head and tiny
    # eye meshes belong on the head texture. Using only the largest-geometry heuristic
    # paints the shirt hem/collar with the head texture, which creates the green strip
    # the user reported. Classify by vertical span instead: meshes whose lowest vertex
    # stays up in the head region use the head texture; everything else uses body.
    slots={}
    for g in geoms:
        gid=g.props[0]
        gverts=np.array(child(g,'Vertices').props[0],float).reshape(-1,3)
        gminy=float(gverts[:,1].min())
        if gminy > 1.35:
            slots[gid]=(0,body_img.height,head_img.width,head_img.height)
        else:
            slots[gid]=(0,0,body_img.width,body_img.height)
            ...
            if raw<0:
                for k in range(1,len(poly)-1):
                    ...
                    for vv,uu in zip(inds,uinds):
                        ...
                        U,V=map(float,uv[uu])
                        U=stable_wrap(U); imageV=max(0.0,min(1.0,stable_wrap(-V)))
                        corner_u.append((x0+0.5+U*(iwid-1))/AW)
                        corner_v.append((y0+0.5+imageV*(ihgt-1))/AH)
                    tri_centers.append(tuple(np.mean(np.array(pts),axis=0)))
                poly=[]; puv=[]

    orders={}
    for key,yaw in {'down':0.,'left':-math.pi/2,'up':math.pi,'right':math.pi/2}.items():
        c,s=math.cos(yaw),math.sin(yaw)
        ds=[(x*s+z*c,i+1) for i,(x,y,z) in enumerate(tri_centers)]
        ds.sort(); orders[key]=[i for _,i in ds]
    ...
    with a.out_lua.open('w',encoding='utf-8',newline='\n') as out:
        out.write('-- GENERATED fresh Shrek Mixamo conversion from supplied Idle/Run/Jump FBXs and replacement textures.\nreturn {\n')
    ...
    print('Shrek Mixamo conversion complete')
    print('geometries',len(geoms),'body_geom',clean_name(next(g for g in geoms if g.props[0]==body_gid)),'bones',len(names),'positions',len(all_bind),'triangles',len(tri_centers),'unit_scale',unit_scale)
